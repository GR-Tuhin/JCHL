  ...
  if (doctors.length) {
    ...
    doctors.forEach(d => {
      ...
      html += `<div class="sr-item" data-action="doctor" data-id="${d.id}">
        ...
        <div class="sr-item-text">
          <div class="sr-item-name">${d.name}</div>
          <div class="sr-item-meta">${dept ? dept.name : ''} · ${d.chamber}</div>
        </div>
      </div>`;
    });
  }
...
function showInfoPanel(part) {
  ...
  els.infoPanel.innerHTML = `
    <div class="panel-hero">
      <span class="panel-hero-icon">${part.icon}</span>
      <div>
  ...
  renderDoctorPanel(`${part.icon} ${part.name} — সংশ্লিষ্ট ডাক্তার`, allDocs);
...
function selectDepartment(deptId) {
  ...
  els.infoPanel.innerHTML = `
    <div class="panel-hero">
      <span class="panel-hero-icon">${dept.icon}</span>
      <div>
  ...
  `;
  els.infoPanel.classList.add("visible");
  renderDoctorPanel(`${dept.icon} ${dept.name} — ডাক্তারগণ`, doctors);
  els.diseaseDetail.classList.remove("visible");
...
function goBackFromDisease() {
  ...
  if (state.currentPart) {
    renderDoctorPanel(
      `${DB.partMap[state.currentPart]?.icon || ''} ${DB.partMap[state.currentPart]?.name || ''} — সংশ্লিষ্ট ডাক্তার`,
...
function renderDoctorPanel(title, doctors) {
  ...
  els.doctorPanel.innerHTML = `
    <div class="dp-header">🩺 ${title} (${doctors.length} জন)</div>
...
function renderDoctorCard(doc) {
  const dept = DB.deptMap[doc.department];
  ...
  return `
    <div class="doctor-card" data-doc-id="${doc.id}">
      ...
      <div class="doc-card-body">
        <div class="doc-dept-tag">${dept ? dept.icon : ''} ${dept ? dept.name : ''}</div>
  ...
  els.profileModal.innerHTML = `
    ...
    <div class="pm-body">
      <div class="pm-section">
        ...
        <span class="pm-dept-badge">${dept ? dept.icon : ''} ${dept ? dept.name : ''}</span>
...
function avatarHTML(doc, large = false) {
  // Try image first; fallback to emoji initials
